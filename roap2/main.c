...
bool check_walls(FILE *fileSaver, int L, int C, int Walls)
{
    int rows = 0, columns = 0, price = 0;
    bool flag = false;
    while (Walls > 0)
    {
        if (fcanf(fileSaver, "%d %d %d", &rows, &columns, &price) != 3)
            exit(0);
        if ((rows == L + 1 && columns == C + 1) || (rows == 1 && columns == 1))
            flag = true;
        Walls--;
    }
    return flag;
...
int main(int argc, char **argv)
{
    ...
    while (fscanf(fp, "%d %d", &L, &C) == 2)
    {
        ...
        // Verifica a posição inicial ou a posição tesouro são o paredes
        if (check_walls(fileSaver, i, j, P))
        {
            fp = fileSaver;
            // ! fprintf("-1\n\n");
            continue;
